...
class VGG16(Model):
    ...
    def forward(self, x):
        x=F.relu(self.conv1_1(x))#(내부적으로 x기반 weight초기화)
        ...
        x=F.reshape(x, (x.shape[0], -1))#for linear. 2차원 텐서로 변환.
    ...
    @staticmethod
    def preprocess(image, size=(224, 224), dtype=np.float32):#이미지를 행렬로 변환하는거래용
