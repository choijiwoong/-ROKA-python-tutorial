...
import numpy as np
from dezero.functions import as_variable, linear
...
def conv2d_simple(x, W, b=None, stride=1, pad=0):
    ...
    OC,C,KH,KW=Weight.shape#output_channel, channel, kernel_height, kernel_width
    SH,SW=pair(stride)#stride_height, stride_width
    PH,PW=pair(pad)#pair_heigh, pair_width
    OH=get_conv_outsize(H,KH,SH,PH)
    OW=get_conv_outsize(W,KW,SW,PW)

    col=im2col(x, (KH,KW), stride, pad, to_matrix=True)#입력데이터를 im2col화
    Weight=Weight.reshape(OC,-1).transpose()#행렬곱을 위한 reshape
    t=linear(col, Weight, b)#곱
    y=t.reshape(N,OH,OW,OC).transpose(0,3,1,2)#형상 복구(기존은 N,OH,OW,OC로 channel이 뒤로가있음_im2col특성상..)
    return y

def pooling_simple(x, kernel_size, stride=1, pad=0):#max
    x=as_variable(x)
    ...
    col=col.reshape(-1, KH*KW)#커널사이즈만큼을 한 행으로
    y=col.max(axis=1)#그중 최고값
    y=y.reshape(N,OH,OW,C).transpose(0,3,1,2)#형상 복구(OH,HW_스트라이드, 패딩 고려 출력 사이즈로 맞춰서)
    return y
